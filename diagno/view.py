...
def appointment(request):
    if request.method == "POST":
        ...
        return render(request, 'diseases.html', {'ds': ds1})
...
def login(request):
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            k = merge.objects.filter(doctor_id=user.pk)
            print(k)
            return redirect('showpatient',{'doc':k})
        else:
            print('invalid' + un + pass1)
    else:
...
def showdr(request):
    x = data1.objects.all()
    return render(request, 'showdr.html',{'x':x})
def showpatient(request):
    # h = patient.objects.get(pk=pk)
    if request.method=='POST':
        # dt =request.POST['date']
        # tm =request.POST['time']
        # treek = date_time(dates=dt,time=tm)
        # print(treek)
        # treek.save()
        x = merge.objects.all()
        return render(request, 'showpatient.html', {'x': x})
        return redirect('index')
...
def paymentMode(request, pk):
    ...
    m=merge(patient_id=current_user.id,doctor_id=ddata.pk,p_email=current_user.email,p_gender=current_user.gender,p_age=current_user.age,p_ph_no=current_user.ph_no,dr_name=ddata.firstname,dr_username=ddata.username,p_disease=current_user.disease,a_date=date.today(),a_time=datetime.now().time(),hospital_name=ddata.hospital)
    m.save()
